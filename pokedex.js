...
async function fetchPokemons(ascending = true) {
  const apiUrl = `https://pokeapi.co/api/v2/pokemon/ditto?offset=${
...
function displayPokemonList(pokemonList) {
  ...
  for (const pokemon of pokemonList) {
    fetch(pokemon.url)
      ...
      .then((pokemonData) => {
        const typeTags = pokemonData.types
          .map((type) => {
        ...
        const imageSize = Math.max(
          150,
          pokemonData.sprites.front_default.length
        );
        ...
        pokemonListHTML += `
                    <div class="pokemon-card">
                        ...
                        <div class="dex-number">#${nationalDexNumber
                          .toString()
                          .padStart(3, "0")}</div>
